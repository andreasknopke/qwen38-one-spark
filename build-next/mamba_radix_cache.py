...
class MambaRadixCache(KVCacheEventMixin, BasePrefixCache):
    def __init__(self, params: CacheInitParams):
        ...
        # qwen38-one-spark port of our upstream PR#38355: upstream dropped the
        # disable_chunked_radix_insert ServerArg, but the chunked-insert race
        # it guarded (prefix KV committed from a still-retractable chunked
        # request, then inherited by the next request -> cross-request
        # prefix poisoning / degenerate loops, seen 2026-09-24 with 4 onsets
        # on 4 rids in 2 min) is still real in this branch. Re-introduced as
        # an env switch so no server_args plumbing is needed.
        self.disable_chunked_radix_insert = (
            os.environ.get("SGLANG_DISABLE_CHUNKED_RADIX_INSERT", "") == "1"
        )
        if self.disable_chunked_radix_insert:
            logger.warning(
                "qwen38-one-spark PR#38355 port ACTIVE: chunked radix insert "
                "disabled (SGLANG_DISABLE_CHUNKED_RADIX_INSERT=1); unfinished "
                "requests defer their prefix insert to cache_finished_req"
            )
        self.kv_event_queue = []
    ...
    def cache_unfinished_req(self, req: Req, chunked=False) -> None:
        ...
        # qwen38-one-spark PR#38355 port: do NOT commit prefix KV while the
        # request is still chunked (i.e. subject to retraction). The insert
        # is deferred to cache_finished_req, which runs after the request is
        # fully committed. Mirrors build/mem_cache/mamba_radix_cache.py:704.
