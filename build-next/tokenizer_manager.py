    ...
    async def _handle_batch_output(
        ...
        for i, rid in enumerate(recv_obj.rids):
            ...
            if state is None:
                ...
                # qwen38-one-spark v8: abort orphaned requests. When the
                # client (proxy/session) is gone, the scheduler kept decoding
                # for 13+ minutes into a deleted stream state (2026-09-25:
                # 6 orphaned rids, 10.833 dropped outputs), binding slots and
                # blocking the v7 auto-flush. TokenizerManager.abort_request()
                # early-returns when rid not in rid_to_state (exactly the
                # orphan case), so dispatch AbortReq directly. One abort per
                # rid; a rid with deleted state will never deliver output
                # again, so there is nothing to protect.
                if os.environ.get("SGLANG_ORPHAN_ABORT", "") == "1":
                    if not hasattr(self, "_v8_orphan_aborted"):
                        self._v8_orphan_aborted = set()
                    if rid not in self._v8_orphan_aborted:
                        self._v8_orphan_aborted.add(rid)
                        logger.error(
                            f"[v8] aborting orphaned request {rid=}: stream "
                            f"state deleted (client gone), request was still "
                            f"decoding server-side"
                        )
                        self._dispatch_to_scheduler(
                            AbortReq(rid=rid, abort_all=False)
                        )
                continue
