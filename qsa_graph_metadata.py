...
def _qsa_ring_width(compress_ratio: int) -> int:
    ...
    `position % width` siga separando micro-bloques completos. Debe coincidir
    con el width que usan pool y metadata (qsa_ring_width.py), o el kernel
    graph y el pool divergen en stride.
    """
    import os
...
def _qsa_graph_row_metadata_kernel(
    ...
    max_pages,
    RATIO: tl.constexpr,
    WIDTH: tl.constexpr,  # pending-ring slots per request (>= RATIO)
    ...
    # Ring slot VALUES use WIDTH (the widened pending-key ring stride),
    # matching the pool reservation and the eager build_pending_ring_slots
    # path (qsa_ring_width.py). Compression arithmetic below stays RATIO.
    tl.store(state_slots_ptr + row, req * WIDTH + (current % WIDTH).to(tl.int64))
    ring_base = row.to(tl.int64) * RATIO
    for k in tl.static_range(RATIO):
        member = tl.maximum(current - (RATIO - 1 - k), 0)
        slot = req * WIDTH + (member % WIDTH).to(tl.int64)
...
def launch_graph_metadata(
    *,
    ...
    _qsa_graph_row_metadata_kernel[(num_rows,)](
        ...
        WIDTH=_qsa_ring_width(indexer.compress_ratio),
